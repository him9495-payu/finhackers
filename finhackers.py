...
# ---------------------------------------------------------------------------
# Pywa WhatsApp integration helpers
# ---------------------------------------------------------------------------

GRAPH_API_URL = (
    f"https://graph.facebook.com/v18.0/{WHATSAPP_PHONE_NUMBER_ID}/messages"
    if META_ACCESS_TOKEN and WHATSAPP_PHONE_NUMBER_ID
    else None
)


def _init_pywa_client() -> Optional[WhatsApp]:
    if not (META_ACCESS_TOKEN and WHATSAPP_PHONE_NUMBER_ID):
        return None
    try:
        return WhatsApp(
        ...
        logger.warning("Failed to initialize pywa client: %s", exc)
        return None


wa_client = _init_pywa_client()


def _graph_post(payload: Dict[str, Any]) -> None:
    if not GRAPH_API_URL or not META_ACCESS_TOKEN:
...
def _call_pywa(method_name: str, **kwargs) -> bool:
    if not wa_client:
        return False
    method = getattr(wa_client, method_name, None)
    ...
    try:
        ...
        logger.warning("pywa.%s failed: %s", method_name, exc)
        return False
...
def send_text(phone: str, body: str) -> None:
    if not body:
        return
    if _call_pywa("send_message", to=phone, text=body):
        return
    if _call_pywa("send_text", to=phone, text=body):
        return
    payload = {
...
def send_buttons(phone: str, body: str, buttons: List[Tuple[str, str]]) -> None:
    normalized_buttons = [
    ...
    if _call_pywa(
        ...
        to=phone,
        body=body,
        buttons=[{"id": button_id, "title": title[:20]} for button_id, title in buttons[:3]],
    ):
        return
    payload = {
        "messaging_product": "whatsapp",
...
def send_flow_message(
    ...
    flow_id = flow_id or WHATSAPP_FLOW_ID
    flow_token = flow_token or WHATSAPP_FLOW_TOKEN
    version = flow_version or WHATSAPP_FLOW_VERSION
    if not (flow_id and flow_token):
        send_text(phone, "Loan form is currently unavailable. Please try again later.")
        return
    if _call_pywa(
        "send_flow",
        to=phone,
        ...
        flow_action_payload={"screen": entry_screen},
        language="en_US" if language == "en" else "hi_IN",
    ):
        return
    payload = {
        "messaging_product": "whatsapp",
...
def send_language_buttons(phone: str) -> None:
    ...
    try:
        send_buttons(phone, english_pack["language_prompt"], buttons)
    except Exception as exc:
...
def start_onboarding(phone: str, state: ConversationState, language: str) -> None:
    ...
    pack = get_language_pack(language)
    send_text(phone, pack["onboarding_intro"])
...
def handle_form_submission(
    ...
    send_text(phone, "It looks like we still need a few details. Please reopen the form.")
    prompt_loan_flow(phone, language)
    record_interaction(
        phone,
        "system",
...
def finalize_onboarding(
    ...
    try:
        ...
        send_text(phone, "Let's collect that information again. Tap Apply to restart the loan journey.")
    ...
    pack = get_language_pack(language)
    send_text(phone, pack["decision_submit"])
...
def handle_support(
    ...
    if bedrock_answer:
        send_text(phone, bedrock_answer)
        send_text(phone, pack["support_closing"])
    ...
    if not answer or confidence < support_agent.threshold:
        send_text(phone, pack["support_handoff"])
        escalate_to_agent(phone, text, profile)
        send_text(phone, pack["support_escalation_ack"])
        record_interaction(
            phone,
            "system",
            "support_escalation",
...
def send_dropoff_message(phone: str, language: str) -> None:
    pack = get_language_pack(language)
    send_text(phone, pack["dropoff"])
    send_text(phone, pack["resume_prompt"])
    record_interaction(
        phone,
        "outbound",
        "dropoff_nudge",
...
def prompt_loan_flow(phone: str, language: str, pack: Optional[Dict[str, str]] = None) -> None:
    ...
    if not WHATSAPP_FLOW_ID:
        ...
        return
    try:
        send_flow_message(phone, language, flow_cta=pack["flow_button_label"])
    except Exception as exc:  # pragma: no cover - flow failures
        logger.warning("Failed to send WhatsApp flow: %s", exc)
        send_text(phone, "I'm having trouble opening the form. Please try again in a moment.")
        return
    send_buttons(
        phone,
        pack["flow_sent"],
...
def prompt_support_menu(phone: str, language: str) -> None:
    ...
    send_text(phone, pack["support_text_hint"])
    record_interaction(
        phone,
        "outbound",
        "support_menu",
    ...
    try:
        ...
        if conversation_state.language is None:
            ...
            if not conversation_state.language_prompted:
                ...
                send_text(phone, reminder_pack["invalid_language"])
                send_language_buttons(phone)
            return
        ...
        if reply_id == "post_accept":
            send_text(phone, pack["accept_ack"])
            try:
                record_interaction(
                    phone,
                    "inbound",
        ...
        if not text:
            send_text(phone, pack["text_only_warning"])
            return
        ...
        if conversation_state.journey == "onboarding":
            ...
            if conversation_state.awaiting_flow_completion:
                send_text(phone, pack["flow_sent"])
                prompt_loan_flow(phone, language, pack)
            else:
                send_text(phone, pack["fallback_intent"])
            return
...
def healthcheck():
    return {
        "status": "ok",
        "messenger_enabled": wa_client is not None,

...
1. **First touch** – chatbot greets the user, requests language preference (English/Hindi) using WhatsApp buttons, and determines whether the customer is new or existing purely from DynamoDB history (no question is asked in chat). If WhatsApp hides the interactive buttons, replying with `1` (English) or `2` (Hindi) selects the language explicitly. At any point customers can type `language` to reopen the selector; this is the only free-text trigger for changing languages.
